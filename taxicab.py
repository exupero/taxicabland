...
class TaxicabGeometry(object):
    ...
    def __init__(self, master):
        ...
        # CANVAS
        c = tk.Canvas(master, width=800, height=600, bg='white')
    ...
    def left_click(self, event):
        self.tool.tool.left_click(event)
        
    def left_drag(self, event):
        self.tool.tool.left_drag(event)
            
    def left_release(self, event):
        self.tool.tool.left_release(event)

    def right_click(self, event):
        self.tool.tool.right_click(event)
            
    def change_tool(self, event):
        for tool in self.toolbox.itervalues():
