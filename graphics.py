...
from utils import current_color, dist, find_intersection, find_intersect
...
def lower_grid(method):
    if method.__name__ != 'lower':
        raise ValueError('`lower_grid` must decorate `lower`')
...
class Graphic(object):
    class __metaclass__(type):
        def __new__(cls, name, bases, attrs):
            ...
            if attrs.get('is_on_top', False):
                if 'update' in attrs:
                    attrs['update'] = lift(attrs['update'])
            elif attrs.get('is_on_bottom', False):
                attrs['__init__'] = lower(attrs.get('__init__', no_op))

            if 'lower' in attrs:
                attrs['lower'] = lower_grid(attrs['lower'])

            if 'update' in attrs:
                attrs['update'] = publish(attrs['update'])
...
class PointOnLine(Point):
    ...
    def __init__(self, line, x, y):
        super(PointOnLine, self).__init__(line)
        self.shape = geometry.PointOnLine(line, x, y)
...
class Circle(Graphic):
    ...
    def __init__(self, center_point, radius_point):
        super(Circle, self).__init__(center_point, radius_point)
...
class Ellipse(Graphic):
    ...
    def __init__(self, focus1, focus2, k_point):
        super(Ellipse, self).__init__(focus1, focus2, k_point)
...
class Hyperbola(Graphic):
    ...
    def lower(self):
        ...
        for handle in self.mid_handles:
            c.lower(handle)

        for handle in self.arm_handles:
            c.lower(handle)
...
class Mindist(Graphic):
    ...
    def __init__(self, points):
        super(Mindist, self).__init__(*points)
